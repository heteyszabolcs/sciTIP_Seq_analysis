...
raw = fread(file = "../data/20230510_EpiLC/count_tables/EpiLC_only_enhancers-k27ac_cluster2-read_counts.tsv")
...
nCount_violin_clusters = VlnPlot(seurat, group.by = "seurat_clusters", features = "nCount_RNA", pt.size = 0.1) +
  ...
  ggtitle("Number of H3.3 counts over EpiLC K27ac cluster 2") +
  ylim(0, 100) +
  xlab("cluster") + 
  ylab("read count") +
  #ylim(0, 60000) +
...
dim = DimPlot(object = seurat, label = FALSE, pt.size = 2, label.size = 7) + 
  ...
  xlim(-8, 8) + 
  ylim(-8, 8) + 
  ggtitle("H3.3 sciTIP-Seq UMAP - EpiLC K27ac cluster 2") +
  theme(
    text = element_text(size = 25),
    plot.title = element_text(size = 10),
...
ggsave(
  glue("{result_folder}Seurat_H3.3_sciTIP_0510-K27ac_cl2_regions.png"),
  plot = last_plot(),
  width = 12,
  height = 5,
  dpi = 300,
...
ggsave(
  glue("{result_folder}Seurat_H3.3_sciTIP_0510-K27ac_cl2_regions.pdf"),
  plot = last_plot(),
  width = 12,
  height = 5,
  device = "pdf"
...
nCount_violin_samples = VlnPlot(seurat, group.by = "sample", features = "nCount_RNA", pt.size = 0.1) +
  ...
  ggtitle("read counts at EpiLC K27ac cluster 2") +
  xlab("sample") + 
  ylab("read count") + 
  #ylim(0, 100) +
...
ggsave(
  glue("{result_folder}Seurat_H3.3_sciTIP_0510-K27ac_cl2_regions-timepoints.png"),
  plot = last_plot(),
  width = 6,
  height = 6,
  dpi = 300,
...
ggsave(
  glue("{result_folder}Seurat_H3.3_sciTIP_0510-K27ac_cl2_regions-timepoints.pdf"),
  plot = last_plot(),
  width = 6,
  height = 6,
  device = "pdf"
